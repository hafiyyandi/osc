...
function setup(){
  createP('Drag & drop your protagonist + setting');
  createSpan("Act: ");
  name_field = createInput("setup");
  name_field.attribute('disabled','disabled');
  people = createSelect();
...
function saveIt(thisDomElement){
  ...
  //console.log(thisElementArray.width);
...
function newElement(elementID,url,x,y,w,h){
  ...
  // var dom_element = createImg(url, function(){
  //   console.log("HERE");
  //   if (w == -1){  //just dropped
  //     w = dom_element.size().width/2;  //pictures tend to be too big
  //     h = dom_element.size().height/2;
  //     console.log("width: "+ dom_element.size().width);
  //   }
  // });

  // for (var i=0; i<100; i++){
  //   console.log("count: "+i+ "  |  width: "+ dom_element.size().width);
  // }
...
function listOfUsers(){
  ...
  data: JSON.stringify( {"distinct": "misc","key": "owner"} ),
  type: "POST",
  contentType: "application/json",
